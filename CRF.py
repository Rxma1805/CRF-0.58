...
class CRF:
    ...
    def make_template(self):
        ...
        for idx, f in enumerate(self.features):
            templatef.write("%d:%%x[0,%d]".encode() % (idx, idx))
        ...
        for idx, t in enumerate(self.template):
            ...
            try:
                ...
                t = (t,)
            templatef.write(("t%d:" % idx + "/".join(["%%x[%d,%d]" % (pi, self.features.index(func))
                                                      for func, pi in t])).encode())
        templatef.flush()
        return templatef
    ...
    def make_feature_input_file(self, sequences, training=False):
        ...
        for sequence in sequences:
            ...
            for idx, item in enumerate(sequence):
                ...
                featureinputf.write(" ".join(fvalues).encode())
            featureinputf.write('\n'.encode())
        featureinputf.flush()
    ...
    def label(self, test_data, labels_only=True):
        ...
        output = subprocess.getoutput("crf_test -m %s %s" % (self.model,
                                                             dataf.name))
    ...
    def evaluate(self, data):
        ...
        for labelseq, goldseq in zip(labels, goldgen()):
...
def tune_cost_parameter(features, template, accuracyfunc,
    ...
    for cost in range(mincost, maxcost + step, step):
...
if __name__ == '__main__':
    ...
    crf_base = "/bigdata/xiaoma/CRF++-0.58"
    ...
    # for label in labels:
    # print label
    #     print(list(c.evaluate(ioseq)))
    print(len(list(c.evaluate(ioseq))))
    print(c.basic_accuracy(list(c.evaluate(ioseq))))
